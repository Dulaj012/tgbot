#!/usr/bin/env python3
"""
Render.com optimized userbot with better phone verification handling
"""
...
# Create client
client = TelegramClient('userbot_render_fixed', TELEGRAM_API_ID, TELEGRAM_API_HASH)
...
async def main():
    ...
    try:
        # Try to start without phone verification first
        print("🔄 Attempting to start with existing session...")
        await client.start()
        print("✅ Userbot started with existing session!")
        
    except Exception as e:
        print(f"❌ Session start failed: {e}")
        print("📱 Phone verification required...")
        print("💡 You need to run this locally first to complete verification:")
        print("   1. Run: python3 userbot_render_fixed.py")
        print("   2. Enter verification code when prompted")
        print("   3. Upload the session file to Render")
        print("   4. Redeploy")
        
        # Try phone verification as fallback
        try:
            await client.start(phone=TELEGRAM_PHONE)
            print("✅ Userbot started with phone verification!")
        except Exception as phone_error:
            print(f"❌ Phone verification failed: {phone_error}")
            print("💡 Please complete verification locally first")
            return
