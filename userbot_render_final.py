#!/usr/bin/env python3
"""
Final Render.com userbot with session file support
"""
...
# Create client - try different session names
session_names = ['userbot_simple_final', 'userbot_local_verification', 'userbot_render_fixed', 'userbot_verification']
client = None

for session_name in session_names:
    try:
        client = TelegramClient(session_name, TELEGRAM_API_ID, TELEGRAM_API_HASH)
        print(f"🔄 Trying session: {session_name}")
        break
    except Exception as e:
        print(f"❌ Session {session_name} failed: {e}")
        continue
...
async def main():
    ...
    try:
        # Try to start without phone verification
        ...
        print("💡 Make sure session files are uploaded to Render")
